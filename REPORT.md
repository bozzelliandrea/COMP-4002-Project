# COMP-4002 Assessment 3 Report

**Name**: Café Sales Analytics & Revenue Prediction
**Group**: 1
**Team**:
...
The dataset used is a café sales record containing 10,000 transactions from a single café across the full year of 2023. The raw data is intentionally dirty, featuring missing values, sentinel strings (`"ERROR"`, `"UNKNOWN"`), and inconsistent formatting, making it a realistic starting point for a full data science workflow.

The project is structured into four stages:

1. **Data preprocessing** — cleaning, imputation, and feature engineering
2. **Exploratory data analysis (EDA)** — understanding temporal patterns, product mix, and customer behaviour
3. **Model selection and training** — building and comparing classification models to predict daily revenue levels
4. **Evaluation and reflection** — comparing model performance and identifying key drivers of predictive accuracy
...
The raw dataset (`dirty_cafe_sales.csv`) contains **10,000 transactions** across 9 columns covering item, quantity, price, payment method, location, date, and time. It is intentionally noisy: missing values are encoded as empty strings, `"ERROR"`, `"UNKNOWN"`, or `"NULL"` rather than standard `NaN`, and numeric columns contain non-numeric sentinel strings. The full preprocessing pipeline is implemented in [`data_preprocessing.ipynb`](../data_preprocessing.ipynb).

## Key Issues & Missing Data

After normalising all sentinel values to `NaN`, the missing-value profile is:
...
Missingness is scattered randomly across rows with no systematic pattern, which validates a column-by-column imputation approach.

## Imputation Strategy

Each column received a tailored strategy based on its semantics:

- **Item** — mode imputation (8 discrete menu items; most frequent is a reasonable default)
- **Quantity** — median imputation (integer values 1–5; robust to mild skew)
- **Price Per Unit** — mean per item, applied in two passes: first where the item is already known, then again after item imputation fills remaining gaps
- **Total Spent** — recalculated as `Quantity × Price Per Unit` after each imputation pass, ensuring internal consistency rather than imputing a raw number
- **Payment Method / Location** — mode imputation (no business rule links them to other columns)
- **Transaction Date** — rows dropped (~4.6 %); dates cannot be reliably reconstructed
- **Transaction Time** — median hour imputation to preserve the realistic time distribution

The two-pass approach for `Price Per Unit` is necessary because some rows are missing both `Item` and `Price Per Unit` simultaneously; filling item first unlocks price imputation for those rows.

After the full pipeline, **zero missing values** remain.

## Cleaning Steps

- **Type casting**: numeric columns cast via `pd.to_numeric(errors="coerce")`; dates via `pd.to_datetime(errors="coerce")`; categorical columns converted to the `category` dtype
- **Duplicate removal**: no full-row duplicates or duplicate `Transaction ID`s were found (9,540 rows retained)
- **Outlier capping**: IQR-based capping applied to `Quantity`, `Price Per Unit`, and `Total Spent`; `Quantity` and `Price Per Unit` had no significant outliers (bounded 1–5 by design); a small number of `Total Spent` values above £25 were capped
...
12 new columns were derived to support modelling:

| Category          | Features                                                |
| ----------------- | ------------------------------------------------------- |
| Date-derived      | `year`, `month`, `day_of_week`, `is_weekend`, `quarter` |
| Time-derived      | `hour`, `time_of_day` (morning / afternoon / evening)   |
| Payment encoding  | `is_cash`, `is_credit_card`, `is_digital_wallet`        |
| Location encoding | `is_takeaway`                                           |

These one-hot encoded flags and temporal features make the dataset immediately usable by ML models requiring numeric input.

## Final Dataset

The cleaned dataset (`cafe_sales_cleaned.csv`) contains **9,540 rows** and **21 columns** (9 original + 12 engineered), with no missing values. It is ready for EDA and modelling.

**Notebook** cafemodel.ipynb
